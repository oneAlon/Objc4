...
attachCategories(Class cls, category_list *cats, bool flush_caches)
{
    ...
    method_list_t **mlists = (method_list_t **)
        malloc(cats->count * sizeof(*mlists)); // 方法数组
    ...
    bool fromBundle = NO;
    // 分类的个数, 注意是--
    // 比如, 编译的分类放到一个数组中, 先编译的先放入数组
    // 这里取得时候是从数组后边取, 取出来以后插入到原来cls的方法列表前
    // 所以是先编译的, 后调用
    while (i--) {
        // 取出一个分类
        ...
        if (mlist) {
            // 将分类中的方法列表, 存放到mlists列表中, mlists是存放着数组的数组
            mlists[mcount++] = mlist;
...
getMethodNoSuper_nolock(Class cls, SEL sel)
{
    ...
    // 查看objc_class类型 可以得知cls->data()其实就是class_rw_t
    // class_rw_t->methods 是方法列表 二维数组
                       ...
                       bool initialize, bool cache, bool resolver)
{
    // cls是给哪一个对象发送的消息
    // sel消息名称
    // inst是cls或者cls子类的一个实例变量
    // initialize = YES , cache = NO , resolver = YES
    
    IMP imp = nil;
    bool triedResolver = NO;// 表示是否已经动态解析方法实现. 在进行动态解析时会置为YES
    ...
    // runtimeLock是在对象在init和realize中间过程中存在, 防止多线程访问.
